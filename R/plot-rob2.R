  ...
  data <- data |>
    dplyr::mutate(
      plot_domain = dplyr::case_when(
        stringr::str_detect(stringr::str_to_lower({{ domain }}), "overall") ~ "Overall risk of bias",
        stringr::str_detect(stringr::str_to_lower({{ domain }}), "allocation|random") ~
          "Bias arising from randomisation process",
        stringr::str_detect({{ domain }}, "deviation|intended") ~ "Bias due to deviations from intended interventions",
        stringr::str_detect(stringr::str_to_lower({{ domain }}), "measure") ~
          "Bias in outcome measurement",
        stringr::str_detect(stringr::str_to_lower({{ domain }}), "miss") ~
          "Bias due to missing outcome data",
        stringr::str_detect(stringr::str_to_lower({{ domain }}), "report|select") ~
          "Bias in selection of reported results"
      )
    ) |>
    dplyr::mutate(
      plot_judgement = dplyr::case_when(
        stringr::str_detect(stringr::str_to_lower({{ judgement }}), stringr::coll("high")) ~
          "High risk of bias",
        stringr::str_detect(stringr::str_to_lower({{ judgement }}), stringr::coll("low")) ~
          "Low risk of bias",
        stringr::str_detect(stringr::str_to_lower({{ judgement }}), stringr::coll("some")) ~
          "Some concerns"
      )
    ) |>
    dplyr::mutate(
